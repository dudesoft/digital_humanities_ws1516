...
public class SampleMath {
   ...
   public static float Vec3FDot(Vec3F v1, Vec3F v2) {
      return v1.getData()[0] * v2.getData()[0] + v1.getData()[1] * v2.getData()[1] +
            v1.getData()[2] * v2.getData()[2];
   }

   public static Vec3F Vec3FNormalize(Vec3F v) {
      float length = (float) Math.sqrt(
            v.getData()[0] * v.getData()[0] + v.getData()[1] * v.getData()[1] +
   ...
   public static Vec3F Vec3FTransform(Vec3F v, Matrix44F m) {
      float lambda;
      lambda = m.getData()[12] * v.getData()[0] + m.getData()[13] * v.getData()[1] +
            m.getData()[14] * v.getData()[2] + m.getData()[15];

      temp[0] = m.getData()[0] * v.getData()[0] + m.getData()[1] * v.getData()[1] +
            m.getData()[2] * v.getData()[2] + m.getData()[3];
      temp[1] = m.getData()[4] * v.getData()[0] + m.getData()[5] * v.getData()[1] +
            m.getData()[6] * v.getData()[2] + m.getData()[7];
      temp[2] = m.getData()[8] * v.getData()[0] + m.getData()[9] * v.getData()[1] +
            m.getData()[10] * v.getData()[2] + m.getData()[11];
   ...
   public static Vec4F Vec4FTransform(Vec4F v, Matrix44F m) {
      temp[0] = m.getData()[0] * v.getData()[0] + m.getData()[1] * v.getData()[1] +
            m.getData()[2] * v.getData()[2] + m.getData()[3] * v.getData()[3];
      temp[1] = m.getData()[4] * v.getData()[0] + m.getData()[5] * v.getData()[1] +
            m.getData()[6] * v.getData()[2] + m.getData()[7] * v.getData()[3];
      temp[2] = m.getData()[8] * v.getData()[0] + m.getData()[9] * v.getData()[1] +
            m.getData()[10] * v.getData()[2] + m.getData()[11] * v.getData()[3];
      temp[3] = m.getData()[12] * v.getData()[0] + m.getData()[13] * v.getData()[1] +
            m.getData()[14] * v.getData()[2] + m.getData()[15] * v.getData()[3];
   ...
   public static Vec3F getPointToPlaneIntersection(Matrix44F inverseProjMatrix,
      ...
      projectScreenPointToPlane(inverseProjMatrix, modelViewMatrix, screenWidth, screenHeight,
            point, planeCenter, planeNormal);
   ...
   public static Vec3F linePlaneIntersection(Vec3F lineStart, Vec3F lineEnd, Vec3F pointOnPlane,
         Vec3F planeNormal) {
   ...
   private static void projectScreenPointToPlane(Matrix44F inverseProjMatrix,
      ...
      float halfViewportWidth = config.getSize()
            .getData()[0] / 2.0f;
      float halfViewportHeight = config.getSize()
            .getData()[1] / 2.0f;
      ...
      mLineStart =
            new Vec3F(nearWorld.getData()[0], nearWorld.getData()[1], nearWorld.getData()[2]);
