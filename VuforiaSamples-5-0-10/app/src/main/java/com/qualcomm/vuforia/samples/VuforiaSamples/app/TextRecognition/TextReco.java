...
public class TextReco extends Activity implements SampleApplicationControl, SampleAppMenuInterface {
   ...
   public boolean doLoadTrackersData() {
      ...
      return wl.loadWordList("TextReco/Vuforia-English-word.vwl", STORAGE_TYPE.STORAGE_APPRESOURCE);
   }
   ...
   public boolean doStartTrackers() {
      ...
      Tracker textTracker = TrackerManager.getInstance()
   ...
   public void onInitARDone(SampleApplicationException exception) {
      ...
      if (exception == null) {
         ...
         mSampleAppMenu = new SampleAppMenu(this, this, "Text Reco", mGlView, mUILayout,
   ...
   void configureVideoBackgroundROI() {
      VideoMode vm = CameraDevice.getInstance()
...

      {
         ...
         int loupeHeight = mIsTablet ? (screenHeight * 10) / 100 : (screenHeight * 15) / 100;
         ...
         mRenderer.setROI(screenWidth / 2, marginWidth + (loupeHeight / 2), loupeWidth,
               loupeHeight);
      }
      ...
      int[] loupeWidth = { 0 };
      int[] loupeHeight = { 0 };
      SampleUtils.screenCoordToCameraCoord((int) mRenderer.ROICenterX, (int) mRenderer.ROICenterY,
            (int) mRenderer.ROIWidth, (int) mRenderer.ROIHeight, screenWidth, screenHeight,
            vm.getWidth(), vm.getHeight(), loupeCenterX, loupeCenterY, loupeWidth, loupeHeight);

      RectangleInt detROI = new RectangleInt(loupeCenterX[0] - (loupeWidth[0] / 2),
            loupeCenterY[0] - (loupeHeight[0] / 2), loupeCenterX[0] + (loupeWidth[0] / 2),
            loupeCenterY[0] + (loupeHeight[0] / 2));

      TextTracker tt = (TextTracker) TrackerManager.getInstance()
            .getTracker(TextTracker.getClassType());
      if (tt != null) {
      ...
      runOnUiThread(new Runnable() {
         ...
         public void run() {
            ...
            if (words.size() > 0) {
               ...
               int[] textInfo =
                     fontSizeForTextHeight(maxTextHeight, words.size(), params.width, 32, 12);
               ...
               for (WordDesc word : words) {
                  ...
                  if (previousView != null) {
                     txtParams.addRule(RelativeLayout.BELOW, previousView.getId());
                  }

                  txtParams.setMargins(0, (count == 0) ? WORDLIST_MARGIN : 0, 0,
   ...
   private int[] fontSizeForTextHeight(int totalTextHeight, int nbWords, int textWidth,
         int textSizeMax, int textSizeMin) {
      ...
      for (textSize = textSizeMax; textSize >= textSizeMin; textSize -= 2) {
         ...
         StaticLayout layout =
               new StaticLayout(text, paint, textWidth, Alignment.ALIGN_NORMAL, spacingMult,
                     spacingAdd, true);
   ...
   private void showLoupe(boolean isActive) {
      ...
      // definition of the region of interest
      mRenderer.setROI(width / 2, marginWidth + (loupeHeight / 2), loupeWidth, loupeHeight);

      // Gets a reference to the loading dialog
      View loadingIndicator = mUILayout.findViewById(R.id.loading_indicator);

      RelativeLayout loupeLayout = (RelativeLayout) mUILayout.findViewById(R.id.loupeLayout);

      ImageView topMargin = (ImageView) mUILayout.findViewById(R.id.topMargin);

      ImageView leftMargin = (ImageView) mUILayout.findViewById(R.id.leftMargin);

      ImageView rightMargin = (ImageView) mUILayout.findViewById(R.id.rightMargin);
      ...
      if (isActive) {
         ...
         params = (RelativeLayout.LayoutParams) loupeLayout.getLayoutParams();
         ...
         params = (RelativeLayout.LayoutParams) wordListLayout.getLayoutParams();
   ...
   private void startLoadingAnimation() {
      ...
      mUILayout = (RelativeLayout) inflater.inflate(R.layout.camera_overlay_textreco, null, false);
