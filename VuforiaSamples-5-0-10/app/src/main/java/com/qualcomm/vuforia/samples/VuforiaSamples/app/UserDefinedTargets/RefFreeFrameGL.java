...
public class RefFreeFrameGL {
   ...
   public RefFreeFrameGL(UserDefinedTargets activity, SampleApplicationSession session) {
      mActivity = activity;
   ...
   boolean init(int screenWidth, int screenHeight) {
      ...
      tempMatrix44Array[0] =
            tempMatrix44Array[5] = tempMatrix44Array[10] = tempMatrix44Array[15] = 1.0f;
      ...
      Configuration config = mActivity.getResources()
            .getConfiguration();
      ...
      if ((shaderProgramID =
            SampleUtils.createProgramFromShaderSrc(frameVertexShader, frameFragmentShader)) == 0) {
         return false;
      }

      if ((vertexHandle = GLES20.glGetAttribLocation(shaderProgramID, "vertexPosition")) == -1) {
         return false;
      }
      if ((textureCoordHandle = GLES20.glGetAttribLocation(shaderProgramID, "vertexTexCoord")) ==
            -1) {
         return false;
      }
      if ((mvpMatrixHandle =
            GLES20.glGetUniformLocation(shaderProgramID, "modelViewProjectionMatrix")) == -1) {
         return false;
      }
      if ((colorHandle = GLES20.glGetUniformLocation(shaderProgramID, "keyColor")) == -1) {
         return false;
      }
   ...
   void renderViewfinder() {
      ...
      GLES20.glDrawElements(GLES20.GL_TRIANGLE_STRIP, NUM_FRAME_INDEX, GLES20.GL_UNSIGNED_SHORT,
            indicesBuffer);
