...
public class TextRecoRenderer implements GLSurfaceView.Renderer {
      ...
      for (int tIdx = 0; tIdx < state.getNumTrackableResults(); tIdx++) {
         ...
         if (result.isOfType(WordResult.getClassType())) {
            ...
            if (wordU != null) {
               ...
               float wordx = -obb.getCenter()
                     .getData()[1];
               float wordy = obb.getCenter()
                     .getData()[0];

               if (mWords.size() < MAX_NB_WORDS) {
                  mWords.add(new WordDesc(wordU, (int) (wordx - wordBoxSize.getData()[0] / 2),
         ...
         Matrix.multiplyMM(mvpMat, 0, vuforiaAppSession.getProjectionMatrix()
               .getData(), 0, mvMat, 0);

         GLES20.glUseProgram(shaderProgramID);
         GLES20.glLineWidth(3.0f);
         GLES20.glVertexAttribPointer(vertexHandle, 3, GLES20.GL_FLOAT, false, 0, mQuadVerts);
         ...
         GLES20.glDrawElements(GLES20.GL_LINES, NUM_QUAD_OBJECT_INDICES, GLES20.GL_UNSIGNED_SHORT,
               mQuadIndices);
   ...
   private void drawRegionOfInterest(float center_x, float center_y, float width, float height) {
      ...
      setOrthoMatrix(0.0f, (float) viewportSize_x, (float) viewportSize_y, 0.0f, -1.0f, 1.0f,
            orthProj);
      ...
      GLES20.glVertexAttribPointer(vertexHandle, 3, GLES20.GL_FLOAT, false, 0, mROIVerts);
      GLES20.glEnableVertexAttribArray(vertexHandle);
   ...
   private void initRendering() {
      ...
      shaderProgramID = SampleUtils.createProgramFromShaderSrc(LineShaders.LINE_VERTEX_SHADER,
            LineShaders.LINE_FRAGMENT_SHADER);

      vertexHandle = GLES20.glGetAttribLocation(shaderProgramID, "vertexPosition");
      mvpMatrixHandle = GLES20.glGetUniformLocation(shaderProgramID, "modelViewProjectionMatrix");

      lineOpacityHandle = GLES20.glGetUniformLocation(shaderProgramID, "opacity");
      lineColorHandle = GLES20.glGetUniformLocation(shaderProgramID, "color");
   }

   private void setOrthoMatrix(float nLeft, float nRight, float nBottom, float nTop, float nNear,
         float nFar, float[] _ROIOrthoProjMatrix) {
      for (int i = 0; i < 16; i++) {
