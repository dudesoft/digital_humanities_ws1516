...
public class SampleApplicationSession implements UpdateCallbackInterface {
   ...
   private class InitVuforiaTask extends AsyncTask<Void, Integer, Boolean> {
      ...
      protected void onPostExecute(Boolean result) {
         ...
         if (result) {
            ...
            if (initTrackersResult) {
               try {
                  ...
                  vuforiaException = new SampleApplicationException(
                        SampleApplicationException.LOADING_TRACKERS_FAILURE, logMessage);
            ...
            vuforiaException =
                  new SampleApplicationException(SampleApplicationException.INITIALIZATION_FAILURE,
                        logMessage);
            mSessionControl.onInitARDone(vuforiaException);
         }
      }
   ...
   public void startAR(int camera) throws SampleApplicationException {
      ...
      if (!CameraDevice.getInstance()
            .selectVideoMode(CameraDevice.MODE.MODE_DEFAULT)) {
   ...
   private void configureVideoBackground() {
      ...
      Log.i(LOGTAG,
            "Configure Video Background : Video (" + vm.getWidth() + " , " + vm.getHeight() +
                  "), Screen (" + mScreenWidth + " , " + mScreenHeight + "), mSize (" + xSize +
                  " , " + ySize + ")");

      Renderer.getInstance()
            .setVideoBackgroundConfig(config);
   }
